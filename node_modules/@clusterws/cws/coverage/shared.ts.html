...
<head>
    <title>Code coverage report for shared.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> shared.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { WebSocket } from './client';
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Number(major) === 13 &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >Number(minor) &lt; 9)</span> {
      // temporary fix for node 13.8 and lower
<span class="cstat-no" title="statement not covered" >      return require(`../dist/bindings/cws_${process.platform}_${process.versions.modules}_8`);</span>
    }
&nbsp;
    return require(`../dist/bindings/cws_${process.platform}_${process.versions.modules}`);
  } catch (err) {
<span class="cstat-no" title="statement not covered" >    err.message = err.message + ` check './node_modules/@clusterws/cws/build_log.txt' for post install build logs`;</span>
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (wsServer.upgradeCb) {
<span class="cstat-no" title="statement not covered" >        wsServer.upgradeCb(socket);</span>
      } else {
...
&nbsp;
  if (type === 'client') {
    native[type].group.onError(group, <span class="fstat-no" title="function not covered" >(w</span>ebSocket: WebSocket): void =&gt; {
<span class="cstat-no" title="statement not covered" >      process.nextTick(<span class="fstat-no" title="function not covered" >(): void =</span>&gt; {</span>
<span class="cstat-no" title="statement not covered" >        webSocket.registeredEvents['error']({</span>
